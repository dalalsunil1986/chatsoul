      	...
      	if(!oncheckFileCall) {
      		...
      		new Ajax.Request('check_file.php',{
      			...
      			onSuccess: function(response){
      				...
      				//incomplete download
      				  {
      					responseMade("Oooops, your file was not completely downloaded.<br /><br />Click <a href='sendform.php'>here</a> to try to send again.");
      					clearInterval(checkFile);
      					}
      					else if (x == 4)
      					{
      					responseMade("Oooops, looks like the receiver for your file has been <b>unresponsive</b> for quite a while.<br /><br />Click <a href='sendform.php'>here</a> to send again.");	
      					clearInterval(checkFile);
      					}
      				else if (x == -1)
      				//code goes here if file is accepted
      				{
      					responseMade("Cool, file transfer was completed.");
      					clearInterval(checkFile);
      					}
      					else if (x == -2)
      					{
      ...
      //handles for general problem
		...
		div.innerHTML = "<p>Oooops, there's a problem sending the file. <br /><br />Click <a href='sendform.php'>here</a> to send again.</p>"
		}
		 //handles for general problem
		function busyuser(){
		var div = document.getElementById('contentWrapper');
		div.innerHTML = "<p>Oooops, looks like the user you're trying to send the file is still busy. <br /><br />Click <a href='sendform.php'>here</a> to try again.</p>"
		}
		
		//handles if no user can be found
		function nouser(){
			var div = document.getElementById('contentWrapper');
		div.innerHTML = "<p>Oooops, looks like there's no receiver for your file. Click <a href='sendform.php'>here</a> to try again.</p>";
			}
		//handles if we can't find a user
		function usererr(a){
				var div = document.getElementById('contentWrapper');
		div.innerHTML = "<p>Oooops, problem connecting with <b>"+a+"</b>. <br /><br />Click <a href='sendform.php'>here</a> to send again.</p>";
		}
